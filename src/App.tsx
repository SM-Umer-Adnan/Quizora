...
>
  <label className="text-sm font-semibold block mb-2 text-gray-900 dark:text-gray-200">
    Time Limit (seconds)
  </label>
  ...
  <motion.input
    ...
    whileFocus={{ scale: 1.02, boxShadow: "0 6px 20px rgba(99,102,241,0.3)" }}
    whileHover={{ scale: 1.01 }}
    className="w-full rounded-xl p-3 placeholder-gray-500 dark:placeholder-gray-400 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-400 transition-all"
    placeholder="Enter time in seconds (e.g. 60)"
  />
</motion.div>
...
<div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border flex justify-between items-center">
  <Timer
    duration={customTime}       
    resetKey={hasGenerated ? quiz.length : 0} 
    onTimeUp={() => setQuizFinished(true)}   
  />
  <div className="font-medium">Score: {score}/{answeredQuestions.filter(Boolean).length} answered</div>
      ...
      <div className="mt-3 flex justify-end">
        <DownloadDropdown quiz={quiz} input={input} userAnswers={userAnswers} />
      </div>
    </div>
